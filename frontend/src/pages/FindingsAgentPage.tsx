...
const SCENARIOS = [
  { id: 'critical_open_finding', label: 'Critical open finding' },
  { id: 'false_positive', label: 'Likely false positive' },
  { id: 'clean_queue', label: 'Clean high/critical queue' },
] as const

export function FindingsAgentPage() {
  const queryClient = useQueryClient()
  const [goal, setGoal] = useState(
    'Triage open high/critical vulnerabilities: inspect the top finding, propose a fix, mark triage, then finish.',
  )
  const [dryRun, setDryRun] = useState(false)
  const [active, setActive] = useState<AgentRun | null>(null)

  const evalMetrics = useQuery({
    queryKey: ['findings-agent-eval'],
    queryFn: () => api<AgentEval>('/api/findings-agent/eval'),
  })
  const runs = useQuery({
    queryKey: ['findings-agent-runs'],
    queryFn: () => api<AgentRun[]>('/api/findings-agent/runs'),
  })

  const runAgent = useMutation({
    mutationFn: () =>
      api<AgentRun>('/api/findings-agent/run', {
        method: 'POST',
        body: JSON.stringify({ goal, dryRun }),
      }),
    onSuccess: (data) => {
      setActive(data)
      void queryClient.invalidateQueries({ queryKey: ['findings-agent-runs'] })
      void queryClient.invalidateQueries({ queryKey: ['findings-agent-eval'] })
    },
  })

  const simulate = useMutation({
    mutationFn: (scenario: string) =>
      api<AgentRun>('/api/findings-agent/simulate', {
        method: 'POST',
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-slate-600">
          Autonomous triage for scan findings — list open vulns, pull code context, propose fixes, and mark
          triage outcomes with an eval loop.
        </p>
      </div>

      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Brain" value={active?.brainType ?? '…'} />
        <Stat
          label="Eval success"
          value={
            evalMetrics.data
        ...
        <Stat label="Avg duration ms" value={evalMetrics.data?.avgDurationMs ?? '—'} />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="cl-panel rounded-2xl p-5">
          ...
          <label className="mb-4 flex items-center gap-2 text-sm text-slate-700">
            ...
            Dry run (skip persisted fix/triage writes)
          </label>
          <button
            type="button"
          ...
          >
            {runAgent.isPending ? 'Agent running…' : 'Run findings agent'}
          </button>
          {runAgent.isError && (
        ...
        <div className="cl-panel rounded-2xl p-5">
          <h2 className="mb-2 text-lg font-semibold">Triage simulator</h2>
          <p className="mb-4 text-sm text-slate-500">
            Seed a known finding state, let the agent choose tools, and score expected triage actions.
          </p>
      ...
      <section className="cl-panel rounded-2xl p-5">
        ...
        <ul className="divide-y divide-slate-100 text-sm">
          ...
          {(runs.data?.length ?? 0) === 0 && (
            <li className="py-3 text-slate-500">No findings-agent runs yet. Start with a simulated triage.</li>
          )}
        </ul>
      </section>
    </div>
