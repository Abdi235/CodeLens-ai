...
type RegisterKind = 'email' | 'username'
...
export function LoginPage() {
  ...
  const [registerKind, setRegisterKind] = useState<RegisterKind>('email')
      ...
      <form onSubmit={onSubmit} className="cl-fade-up-delay cl-panel rounded-2xl p-7">
        ...
        {mode === 'register' && (
          <div className="mb-4 grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              onClick={() => setRegisterKind('username')}
              className={`rounded-xl px-3 py-2 text-sm font-medium transition ${
                registerKind === 'username'
                  ? 'bg-teal-50 text-teal-900 ring-1 ring-teal-200'
                  : 'bg-white text-slate-600 ring-1 ring-slate-200'
              }`}
            >
        ...
        {mode === 'register' && registerKind === 'email' && (
        ...
        {mode === 'register' && registerKind === 'username' && (
          <label className="mb-4 block text-sm font-medium text-slate-700">
            Username
            <input
              type="text"
        ...
        {mode === 'login' && (
          <label className="mb-4 block text-sm font-medium text-slate-700">
            ...
            <input
              ...
              className="cl-input mt-1.5 w-full rounded-xl px-3.5 py-2.5"
              placeholder="you@example.com or codelens_user"
              autoComplete="username"
            />
          </label>
        )}
